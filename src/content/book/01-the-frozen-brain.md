...
src: "part-1-the-problem/01-the-frozen-brain.md"
kind: "chapter"
---
I own a robot vacuum. Every morning it leaves its dock, drives into the same
chair leg, backs up, turns a little, and gets on with its day. It has hit
that chair leg hundreds of times. It will hit it again tomorrow.

This bothers me more than it should. The chair is fine. What bothers me is
what the bump proves: somewhere in that vacuum is a brain, and that brain is
not learning. It can't. It was finished before the vacuum ever saw my living
room.

Here is how most machine brains are made today. First comes a long study
phase, in a lab, on enormous piles of examples. The brain gets better and
better at its task. Then, one day, the studying stops. Whatever the brain
knows at that moment gets copied into the product. From that point on,
nothing it experiences changes it. Engineers call the study phase *training*.
I call the copy that ships a *frozen* brain: a snapshot of what was learned,
with the learning switched off.

Nobody would accept this for a person: it would mean hiring someone whose
last day of learning was their final exam. We accept it for almost every
machine.
...
copy you ship behaves exactly like the copy you tested. It's cheap: train
once, stamp out a million identical copies. And when it makes a mistake, it
makes the same mistake every time, so an engineer can hunt the mistake down
and fix it in the next version. A brain that keeps changing has none of
these comforts. There's a reason nobody wants their bank's software to
improvise.

So if the world the machine lives in matches the world it studied, frozen is
fine. A barcode scanner doesn't need to grow.

The trouble starts when anything moves. And three things move all the time.
...
its rubber. Sensors drift out of calibration. My vacuum's spinning brush is
a little balder every month, so the same motor command sweeps a little less
...
confidently doing the wrong thing. My vacuum will bump the chair every day
until its maker ships a smarter model, which is to say: forever.

What I want is different, and it's the reason this book exists. I want a
...
the way it is for anything alive. A puppy isn't trained and then deployed.

Picture what that would mean for the vacuum. Monday, it hits the chair leg.
Tuesday, it hits the chair leg and something in it registers: that spot,
that turn, a bump again. Wednesday it swings a touch wider. By Friday it
slides past without touching, and nobody told it anything: no update was
downloaded, no engineer was involved. If the chair moves next month, it
bumps a few times and adjusts again. None of this is science fiction; a
mouse does it effortlessly. The reason your vacuum doesn't is a design
...
rovers to anything that speaks a robot's message protocol. This book is the
...
Part 5, which claims haven't faced their attempt yet.

You won't have to take my word for any of it, either. The system is open
source, and in Part 4 you'll install it yourself and watch a small rover's
...
and Part 5 takes that idea seriously. But language would be one more thing
